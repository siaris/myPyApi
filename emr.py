...
def get_erm_result(code,v='',limit=''):
    ...
    return exec_q("select json_result from mr_result inner join mr_form on \
...
def get_attr_emr(attrs):
  ...
  for i in attrs:
    ...
    for j in attr['emr']:
      EMRid.append(j)
  mycursor.execute('select mr_result.id,json_result,kode,user_created from mr_result inner join mr_form on \
